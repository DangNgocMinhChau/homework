const listQuestion = [
    ...
    {
        question: "Thành phố nào được mệnh danh là 'Thành phố ngàn hoa' ở Việt Nam?",
        answers: [
            "A. Đà Lạt",
            "B. Hội An",
            "C. Huế",
    ...
    {
        question: "Loại trái cây nào được coi là vua của các loại trái cây ở Đông Nam Á?",
        answers: [
            "A. Sầu riêng",
            "B. Măng cụt",
            "C. Chuối",
        ...
        if (selectedAnswer) {
            if (selectedAnswer.innerText === q.correctAnswer) {
                ...
                if (currentQuestionIndex < listQuestion.length - 1) {
                    ...
                    document.querySelectorAll('.questions')[currentQuestionIndex].classList.add('active');
                } else {
...
function startGame() {
    playerName = prompt('Nhập tên của bạn (từ 5 đến 10 ký tự, không có số hoặc ký tự đặc biệt):');
    if (playerName === null) {
    ...
    if (isValidName(playerName)) {
        ...
        startGame(); // Gọi lại để người dùng nhập lại tên
    }
}
